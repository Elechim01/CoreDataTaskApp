...
//                Date Picker ...
            ...
            .padding()
//            disabiling button when no data...
            .disabled(homeData.content == "" ? true: false)
            .opacity(homeData.content == "" ? 0.5 : 1)
        }
        .background(Color.black.opacity(0.10).ignoresSafeArea(.all,edges: .bottom))
    }
}

struct NewDataView_Previews: PreviewProvider {
    static var previews: some View {
        NewDataView(homeData: HomeViewModel()).environmentObject(HomeViewModel())
    }
}
